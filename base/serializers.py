...
class LikeSerializer(serializers.ModelSerializer):
    ...
    liker_avi = serializers.URLField(read_only=True)
...
class FollowRequestSerializer(serializers.ModelSerializer):
    avi = serializers.URLField(read_only=True)
...
class BookmarkSerializer(serializers.ModelSerializer):
    ...
    booker_avi = serializers.URLField(read_only=True)
...
class CommentSerializer(serializers.ModelSerializer):
    comment_avi = serializers.URLField(read_only=True)
...
class PostSerializer(serializers.ModelSerializer):
    ...
    user_avi = serializers.URLField(read_only=True)
...
class FollowersSerializer(serializers.ModelSerializer):
    follower_avi = serializers.URLField(read_only=True)
    ...
    def validate(self, attrs):
        ...
        if user.is_verified:
            ...
            if Userr.objects.filter(email=email).exists():
                ...
                token = PasswordResetTokenGenerator().make_token(user)
                request = self.context.get('request')
                abslink = f"https://galleria.pythonanywhere.com/#/password-reset-confirm/{uidb64}/{token}/"
                print(abslink)
                email_body = f"Hi {user.first_name}, use the link below to reset your password: {abslink} Hurry Up The Link Expires in Two Minutes"
                data = {
                    'email_body': email_body,
